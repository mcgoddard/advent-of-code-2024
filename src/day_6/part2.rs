use std::collections::HashSet;

use super::lib::{parse_input, Direction, Space};

pub fn part2(lines: Vec<String>) -> i64 {
  let (start_position, start_direction, map) = parse_input(&lines);
  let mut position = start_position;
  let mut direction = start_direction.clone();
  let valid_positions = map.iter().enumerate().map(|(y_blocker, line)| {
    line.iter().enumerate().map(|(x_blocker, space)| {
      if (x_blocker == start_position.0 as usize && y_blocker == start_position.1 as usize) || space == &Space::Blocked {
        return false;
      }
      position = start_position;
      direction = start_direction.clone();
      let mut visited: HashSet::<(i64, i64, Direction)> = HashSet::new();
      let mut new_map = map.iter().map(|line| line.iter().map(|space| space).cloned().collect()).collect::<Vec<Vec<Space>>>();
      new_map[y_blocker][x_blocker] = Space::Blocked;
      while position.0 >= 0 && position.0 < lines[0].len() as i64 && position.1 >= 0 && position.1 < lines[0].len() as i64 {
        if !visited.contains(&(position.0, position.1, direction.clone())) {
          visited.insert((position.0, position.1, direction.clone()));
        } else {
          return true;
        }
        let new_position = match direction {
          Direction::Up => (position.0, position.1 - 1),
          Direction::Down => (position.0, position.1 + 1),
          Direction::Left => (position.0 - 1, position.1),
          Direction::Right => (position.0 + 1, position.1),
        };
        if new_position.0 >= 0 && new_position.0 < lines[0].len() as i64 && new_position.1 >= 0 && new_position.1 < lines[0].len() as i64 {
          match new_map[new_position.1 as usize][new_position.0 as usize] {
            Space::Blocked => {
              direction = match direction {
                Direction::Up => Direction::Right,
                Direction::Right => Direction::Down,
                Direction::Down => Direction::Left,
                Direction::Left => Direction::Up,
              };
            },
            Space::Open => {
              position = new_position;
            }
          }
        } else {
          position = new_position
        }
      }
      return false;
    }).collect::<Vec<bool>>()
  }).flatten().filter(|v| *v).collect::<Vec<bool>>();
  valid_positions.len() as i64
}
